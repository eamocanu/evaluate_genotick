...
def get_truth_from_data(data_dir, skip_reverse = False, price_col=0, new_offset=2, old_offset=1, save_dir=None, get_dataframe=True):
    ...
    for fn in os.listdir(data_dir):
        ...
        with open(os.path.join(data_dir, fn), 'r') as fr:
            fw = open(os.path.join(save_dir, fn), 'w', newline='\n') if save_output else None
            csvr = csv.reader(fr)
            ...
            while not end:
                ...
                if get_dataframe: df.append(row)

            if get_dataframe: output[fn] = df

            if save_output: fw.close()

    if get_dataframe: 
        for name in output:
            output[name] = pandas.DataFrame(data=output[name], columns=['TimePoint', 'Class','Delta']) # Delta is unused in rest of project
...
def read_truth_from_dir(truth_dir, skip_reverse = False):
    ...
    for fn in os.listdir(truth_dir):
        if skip_reverse and fn.lower().startswith('reverse_'): continue
        output[fn] = pandas.read_csv(os.path.join(truth_dir, fn), header=None, names=['TimePoint', 'Class', 'Delta'], index_col=0)
